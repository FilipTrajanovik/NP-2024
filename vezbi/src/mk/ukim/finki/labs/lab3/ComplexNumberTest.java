...
class ComplexNumber<T extends Number, U extends Number> implements Comparable<ComplexNumber<?,?>>{
    ...
    public double modul(){
        ...
        return Math.sqrt(
                Stream.of(real.doubleValue(), imaginary.doubleValue())
                        .mapToDouble(x -> x * x)
                        .sum()
        );
    }
    ...
    public String toString() {
        if (imaginary.doubleValue() < 0) {
            return String.format("%.2f%.2fi", real.doubleValue(), imaginary.doubleValue()); // нема потреба за плус-симбол
        } else {
            return String.format("%.2f+%.2fi", real.doubleValue(), imaginary.doubleValue()); // додава плус-симбол
        }
    }
}