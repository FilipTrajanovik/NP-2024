...
class Faculty{
    ...
    public void addRecord(String studentId, String courseName, int grade, LocalDate timestamp) {
        Record r = new Record(studentId, courseName, grade, timestamp);
        records.add(r);
    }
    ...
    public Map<String, Long> studentsPassedCoursesCount(){
        return records.stream().filter( record -> record.getGrade() > 5).collect(Collectors.groupingBy(
                Record::getStudentId, TreeMap::new, Collectors.counting()
        ));
    }
    ...
    public Map<String, List<String>> studentsPassedCourses(){
        return records.stream().filter(record -> record.getGrade() > 5).collect(Collectors.groupingBy(
                Record::getStudentId, TreeMap::new, Collectors.mapping(Record::getCourseName, Collectors.toList())
        ));
    }
