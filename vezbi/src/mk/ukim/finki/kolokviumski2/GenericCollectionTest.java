...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    private Map<String, Set<T>> categoryToElements;
    private Map<String, Set<T>>timestampToElements;


    public GenericCollection() {
        categoryToElements = new HashMap<>();
        timestampToElements = new TreeMap<>();
    }


    public void addGenericItem(String category, T element) {
        categoryToElements.putIfAbsent(category, new TreeSet<>(Comparator.reverseOrder()));
        categoryToElements.get(category).add(element);

        String key = String.format("%02d-%02d",element.getTimestamp().getMonth().getValue(),element.getTimestamp().getDayOfMonth());
        timestampToElements.putIfAbsent(key,new TreeSet<>(Comparator.reverseOrder()));
        timestampToElements.get(key).add(element);

    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return categoryToElements.values().stream()
                .flatMap(Set::stream)
                .filter(t -> (t.getTimestamp().isAfter(from) && t.getTimestamp().isBefore(to)) || t.getTimestamp() == from || t.getTimestamp() == to)
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());
    }

    public Collection<T> itemsFromCategories(List<String> categories) {
        return categoryToElements.entrySet().stream()
                .filter(me -> categories.contains(me.getKey()))
