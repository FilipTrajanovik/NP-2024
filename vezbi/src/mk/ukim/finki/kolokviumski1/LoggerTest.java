...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = elements -> elements.stream().filter(element -> element.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = elements -> elements.stream().filter(element -> element.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor <T>thirdLogProcessor = (logsList) -> logsList.stream().sorted((s1, s2)->{
