...
class PriorityTask implements Task {
    ...
    public String toString() {
        return String.format("PT -> %d", getOrdered());
    }
...
class TimedTask implements Task {
    ...
    @Override
    public String toString() {
        return String.format("TT -> %d", getOrdered());
    }
...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        ...
        return task -> Arrays.stream(task).sorted(Comparator.comparing(Task::getOrdered)).collect(Collectors.toList());
    }

    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {

        // vashiot kod ovde (lambda expressio   n)
        return task -> Arrays.stream(task).filter(t -> t.getOrdered() <= order).collect(Collectors.toList());
    }
}