...
class RuleProcessor{
    public static <IN, OUT> void process(List<IN> inputs, List<Rule<IN, OUT>> rules)
    {
        for (IN input : inputs) {
            ...
            {
                Optional<OUT> output = rule.apply(input);
                if(output.isPresent()){
                    System.out.println("Result: " + output.get());
                }else{
...
public class RuleTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            rules.add(new Rule<>(s -> s.contains("NP"), s->s.indexOf("NP")));
            ...
            * */
            ...
            rules.add(new Rule<>(student -> student.grades.size() >=3 ,student -> student.grades.stream().mapToDouble(Integer::doubleValue).max().getAsDouble()));
